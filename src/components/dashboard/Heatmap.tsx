...
export default function Heatmap({ dashboardData }: HeatmapProps) {
  ...
  return (
    <section className="w-full p-4 bg-white rounded-xl border">
              ...
              return {
                ...
                'data-tooltip-content':
                  activity && activity.date
                    ? `${activity.date} : ${activity.count}개`
                    : '데이터 없음',
              } as any;
            }}
          />
      ...
      <div className="flex justify-end items-center gap-2 md:gap-3 text-[10px] sm:text-xs text-muted-foreground -mt-6 sm:-mt-12">
        <span className="font-medium">Less</span>
        <div className="flex gap-1">
          <div className="w-4 h-4 rounded-[2px] bg-[#ebedf0] opacity-80" />
